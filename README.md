...
* `Event Tickets` - "The Events Calendar" or "Event Tickets"
* `Default Ticket Fieldset` - Plugin Human-readable name, e.g. "Sample Extension"
* `Default_Ticket_Fieldset` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `default-ticket-fieldset` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `default_ticket_fieldset` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
